...
class Helper:
    ...
    def __init__(self, parse_arguments_fn):
        ...
        # Configure logging (console minimal vs full)
        ...
        # set_logging (file handler always receives full logs)
        ...
        # Neptune (optional)
        neptune_logger = None
        if args.official_run and neptune is not None and credentials is not None:
        ...
        # log_properties (filtered on console in minimal mode, full in file)
        args_dict = vars(args)
    ...
    def _configure_logging(self, args):
        """Configure console and file logging. In minimal mode, console only shows essential lines."""
        ...
        # Minimal console filter
        log_mode = getattr(args, "log_mode", "minimal")
        if log_mode == "minimal":
            patterns = [
                "Device:",
                "CUDA available:",
            ...
            class _MinimalFilter(logging.Filter):
                def filter(self, record: logging.LogRecord) -> bool:
                    msg = record.getMessage()
                    return any(p in msg for p in patterns)

            console.addFilter(_MinimalFilter())
    ...
    def get_device(no_cuda=False):
        ...
        cuda = cuda_is_avail and (not no_cuda)
        device = torch.device("cuda" if cuda else "cpu")
        logging.info(f"CUDA available: {cuda_is_avail}")
